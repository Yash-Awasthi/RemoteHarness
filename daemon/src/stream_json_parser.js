 ...
 *
 * NOTE: this file was ported from TS-syntax-in-.js to plain ESM (it could not
 * be imported under the package's "type": "module" before).
 */
...
export class StreamJsonParser {
   ...
   * @param {string} line
   * @returns {object|null} parsed message or null when blank/unparseable
   */
  parseLine(line) {
  ...
  formatSystemMessage(event, base) {
  ...
  formatAssistantMessage(event, base) {
   ...
   * @returns {{messageCount: number, totalInputTokens: number, totalOutputTokens: number, pendingTools: number}}
   */
  getStats() {
    return {

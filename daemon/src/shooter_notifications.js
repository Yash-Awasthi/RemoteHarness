...
export class ShooterNotifications extends EventEmitter {
  constructor(config = {}) {
    super();
    this.telemetry = [];
  ...
  sendNotification(event) {
    ...
    // Normal-priority events inside the coalesce window are held (coalesced)
    // rather than sent — the burst carries them until the window expires.
    // (High-priority/decision-first events always pass through.)
    if (decision.priority !== "high" && Date.now() - burst.lastSent < this.config.coalesceWindowMs) {
    ...
    // Send the notification. High-priority (decision-first) sends do NOT arm
    // the coalesce window — only routine sends start the quiet period.
    burst.events = [];
    if (decision.priority !== "high") burst.lastSent = Date.now();
    this.projectBursts.set(projectId, burst);
